...
if( !function_exists('clear_company_cache') ){
	function clear_company_cache(int $companyId): void {
		$cache = \Config\Services::cache();
		$cache->delete('company_settings_' . $companyId);
	}
}

// Clear system settings cache (call on system settings save)
if( !function_exists('clear_system_cache') ){
	function clear_system_cache(): void {
		$cache = \Config\Services::cache();
		$cache->delete('system_settings');
	}
}

// Clear role cache for a user (call on role change)
if( !function_exists('clear_role_cache') ){
	function clear_role_cache(int $userId): void {
		$cache = \Config\Services::cache();
		$cache->delete('super_role_' . $userId);
		$cache->delete('staff_role_' . $userId);
	}
}
...
if( !function_exists('set_date_format') ){
	...
	function set_date_format($date) {
		...
		if($user_info['user_type'] == 'super_user'){
			...
			if($com_xin_system['datepicker_locale']=='ar-sa'){
				$d_format = $date;
			} else if($com_xin_system['datepicker_locale']=='ar'){
				$d_format = $date;
			} else if($com_xin_system['datepicker_locale']=='fa'){
				$d_format = $date;
			} else if($com_xin_system['datepicker_locale']=='hi'){
				$d_format = $date;
			} else if($com_xin_system['datepicker_locale']=='ne'){
				$d_format = $date;
			} else if($com_xin_system['datepicker_locale']=='ta-in'){
				$d_format = $date;
			} else {
				// date format
				if($date!='') {
