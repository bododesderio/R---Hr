...
class Recruitment extends BaseController {
	...
	public function candidates_list() {
		...
		//	$date_of_closing = set_date_format($r['date_of_closing']);
			...
			if($staff_info){
				$istaff_info = $staff_info['first_name'].' '.$staff_info['last_name'];
				$istaff_email = $staff_info['email'];
			} else {
				$istaff_info = '--';
				$istaff_email = '--';
			}
	...
	public function add_job() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$job_title = strip_tags(trim($this->request->getPost('job_title')));
				$job_type = strip_tags(trim($this->request->getPost('job_type')));
				$designation_id = strip_tags(trim($this->request->getPost('designation_id')));
				$vacancy = strip_tags(trim($this->request->getPost('vacancy')));
				$date_of_closing = strip_tags(trim($this->request->getPost('date_of_closing')));
				$short_description = strip_tags(trim($this->request->getPost('short_description')));
				$long_description = strip_tags(trim($this->request->getPost('long_description')));
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			if($this->request->getPost('status') == 3){
				...
				if(!$this->validate($rules)){
					...
					$interview_date = strip_tags(trim($this->request->getPost('interview_date')));
					$interview_time = strip_tags(trim($this->request->getPost('interview_time')));
					$interview_place = strip_tags(trim($this->request->getPost('interview_place')));
					$interviewer_id = strip_tags(trim($this->request->getPost('interviewer_id')));
	...
	public function apply_job() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$cover_letter = strip_tags(trim($this->request->getPost('cover_letter')));
				$id = udecode(strip_tags(trim($this->request->getPost('token'))));
