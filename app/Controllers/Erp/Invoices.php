...
class Invoices extends BaseController {
	...
	// server-side list
	public function invoices_list_server() {
		...
		$builder = $db->table('ci_invoices')
			->select('ci_invoices.*, ci_projects.title as project_title')
			->join('ci_projects', 'ci_projects.project_id = ci_invoices.project_id', 'left')
			->where('ci_invoices.company_id', $company_id);

		if (!empty($search)) {
			$builder->groupStart()
				->like('ci_invoices.invoice_number', $search)
				->orLike('ci_projects.title', $search)
				->groupEnd();
		}

		$total    = $builder->countAllResults(false);
		$filtered = $builder->countAllResults(false);
		$data     = $builder->limit($length, $start)->orderBy('ci_invoices.invoice_id', 'ASC')->get()->getResultArray();
	...
	public function create_new_invoice() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$invoice_note = strip_tags(trim($this->request->getPost('invoice_note')));
	...
	public function pay_invoice_record() {
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			if(!$this->validate($rules)){
				...
				$payment_method = strip_tags(trim($this->request->getPost('payment_method')));	
