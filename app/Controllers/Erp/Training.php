	...
	public function add_training() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$trainer = strip_tags(trim($this->request->getPost('trainer')));
				$training_type = strip_tags(trim($this->request->getPost('training_type')));
				$training_cost = strip_tags(trim($this->request->getPost('training_cost')));
				...
				$module_id = $TrainingModel->insertID();
				$Return['csrf_hash'] = csrf_hash();	
				if ($result == TRUE) {
	...
	public function update_training_status() {
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			if(!$this->validate($rules)){
				...
				$performance = strip_tags(trim($this->request->getPost('performance')));	
