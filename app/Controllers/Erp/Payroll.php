...
class Payroll extends BaseController {
	...
	// server-side list
	public function payroll_list_server() {
		...
		$builder = $db->table('ci_payslips')
			->select('ci_payslips.*, ci_erp_users.first_name, ci_erp_users.last_name, ci_erp_users.email, ci_erp_users.profile_photo')
			->join('ci_erp_users', 'ci_erp_users.user_id = ci_payslips.staff_id', 'left')
			->where('ci_payslips.company_id', $company_id);

		if (!empty($search)) {
			$builder->groupStart()
				->like('ci_erp_users.first_name', $search)
				->orLike('ci_erp_users.last_name', $search)
				->orLike('ci_payslips.salary_month', $search)
				->groupEnd();
		}

		$total    = $builder->countAllResults(false);
		$filtered = $builder->countAllResults(false);
		$data     = $builder->limit($length, $start)->orderBy('ci_payslips.payslip_id', 'ASC')->get()->getResultArray();
	...
	public function add_advance_salary() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$advance_amount = strip_tags(trim($this->request->getPost('advance_amount')));
				$one_time_deduct = strip_tags(trim($this->request->getPost('one_time_deduct')));
				$reason = strip_tags(trim($this->request->getPost('reason')));
				// get one time value
				if($one_time_deduct==1){
					$emi_amount = $advance_amount;
				} else {
					$iemi_amount = strip_tags(trim($this->request->getPost('emi_amount')));
					if($iemi_amount == ''){
						$emi_amount = 0;
					} else {
						$emi_amount = strip_tags(trim($this->request->getPost('emi_amount')));
					}
				}
				...
				if($user_info['user_type'] == 'staff'){
					...
					$assigned_ids = strip_tags(trim($this->request->getPost('employee_id')));
	...
	public function add_pay_monthly() {
		...
		if ($this->request->getPost('type') === 'add_monthly_payment') {
			...
			$id = udecode(strip_tags(trim($this->request->getPost('token'))));
			$salary_month = udecode(strip_tags(trim($this->request->getPost('token2'))));
			$MainModel = new MainModel();
			$ContractModel = new ContractModel();
			$AdvancesalaryModel = new AdvancesalaryModel();
			...
			if($advance_salary > 0) {
				$fadvance_salary = $AdvancesalaryModel->where('employee_id',$id)->where('status',1)->where('salary_type','advance')->first();
				if($fadvance_salary['advance_amount']!= $fadvance_salary['total_paid']){
					$monthly_installment = $fadvance_salary['monthly_installment'];
					$advance_amount = $fadvance_salary['advance_amount'];
					$total_paid = $fadvance_salary['total_paid'];
			...
			if($lo_advance_salary > 0) {
				$flo_advance_salary = $AdvancesalaryModel->where('employee_id',$id)->where('status',1)->where('salary_type','loan')->first();
				if($flo_advance_salary['advance_amount']!= $flo_advance_salary['total_paid']){
					$lo_monthly_installment = $flo_advance_salary['monthly_installment'];
					$lo_advance_amount = $flo_advance_salary['advance_amount'];
					$lo_total_paid = $flo_advance_salary['total_paid'];
			...
			$ibasic_salary = $user_detail['basic_salary'];
			// Uganda PAYE and NSSF tax calculations
			$taxEngine = new TaxEngine();
			$grossForTax = $ibasic_salary + $allowance_amount + $commissions_amount + $other_payments_amount;
			$taxDeductions = $taxEngine->calculateDeductions($grossForTax, $company_id);
			$nssf_employee = $taxDeductions['nssf_employee'];
			$nssf_employer = $taxDeductions['nssf_employer'];
			$paye_tax = $taxDeductions['paye'];
			// net salary (includes statutory deductions + PAYE + NSSF employee share)
			$inet_salary = $ibasic_salary + $allowance_amount + $commissions_amount + $other_payments_amount - $statutory_deductions_amount - $nssf_employee - $paye_tax;
			// add info
			$payslip_comments = strip_tags(trim($this->request->getPost('payslip_comments')));
			$data = [
