...
class Projects extends BaseController {
	...
	public function add_project() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$client_id = strip_tags(trim($this->request->getPost('client_id')));
				...
				if ($result == TRUE) {
					...
					if($xin_system['enable_sms_notification'] == 1){
						$stemplate = $SmstemplatesModel->where('template_id', 1)->first();
						$sbody = html_entity_decode($stemplate['message']);
						foreach($this->request->getPost('assigned_to') as $_staff_id){
							$staff_info = $UsersModel->where('user_id', $_staff_id)->first();
							$fbody = str_replace(array("{firstname}","{project_name}"),array($staff_info['first_name'],$title),$sbody);
	...
	public function update_project_progress() {
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			if(!$this->validate($rules)){
				...
				$progres_val = strip_tags(trim($this->request->getPost('progres_val')));
	...
	public function add_timelogs() {
		...
		if ($this->request->getPost('type') === 'add_timelogs') {
			...
			if(!$this->validate($rules)){
				...
				$start_time = strip_tags(trim($this->request->getPost('start_time')));
				$end_time = strip_tags(trim($this->request->getPost('end_time')));
				$start_date = strip_tags(trim($this->request->getPost('start_date')));
				$end_date = strip_tags(trim($this->request->getPost('end_date')));
				$memo = strip_tags(trim($this->request->getPost('memo')));
				$id = udecode(strip_tags(trim($this->request->getPost('token'))));
				// total hours
	...
	public function add_attachment() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$file_title = strip_tags(trim($this->request->getPost('file_name')));
				$id = udecode(strip_tags(trim($this->request->getPost('token'))));
				$UsersModel = new UsersModel();
				$user_info = $UsersModel->where('user_id', $usession['sup_user_id'])->first();
				if($user_info['user_type'] == 'staff'){
	...
	public function update_project_status() {
		...
		if($this->request->getVar('xfieldid')) {
			...
			$request = \Config\Services::request();
			$usession = $session->get('sup_username');
			$id = strip_tags(trim($this->request->getVar('xfieldid')));
			$status = strip_tags(trim($this->request->getVar('xfieldst')));
			$Return['csrf_hash'] = csrf_hash();
			$UsersModel = new UsersModel();
			$user_info = $UsersModel->where('user_id', $usession['sup_user_id'])->first();
			if($user_info['user_type'] == 'staff'){
