...
/*		if($user_info['user_type'] != 'company' && $user_info['user_type']!='staff'){
	...
	public function add_todo() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$task_insert = strip_tags(trim($this->request->getPost('task_insert')));			
				$UsersModel = new UsersModel();
				$user_info = $UsersModel->where('user_id', $usession['sup_user_id'])->first();
	...
	public function delete_todo() {
		...
		if($this->request->getVar('field_id')) {
			...
			$id = strip_tags(trim($request->getVar('field_id')));
			$Return['csrf_hash'] = csrf_hash();
			$TodoModel = new TodoModel();
			$UsersModel = new UsersModel();
			$user_info = $UsersModel->where('user_id', $usession['sup_user_id'])->first();
	...
	public function update_item() {
		...
		if($this->request->getVar('field_id')) {
			...
			//$id = udecode($this->request->getGet('field_id'));
			$id = strip_tags(trim($request->getVar('field_id')));
			$is_done = strip_tags(trim($request->getVar('task_done')));
			$Return['csrf_hash'] = csrf_hash();
			$TodoModel = new TodoModel();
			$UsersModel = new UsersModel();
			$user_info = $UsersModel->where('user_id', $usession['sup_user_id'])->first();
