...
class Mailbox extends BaseController {
	...
	public function send_mail() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$staff_id = strip_tags(trim($this->request->getPost('staff_id')));	
	...
	public function reply_mail() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$description = strip_tags(trim($this->request->getPost('description')));	
				$mail_id = udecode(strip_tags(trim($this->request->getPost('token'))));
				$mail_to = udecode(strip_tags(trim($this->request->getPost('token2'))));
	...
	public function update_starmail_record() {
		...
		if($this->request->getVar('field_id')) {
			...
			$id = udecode(strip_tags(trim($request->getVar('field_id'))));
			$star_val = strip_tags(trim($request->getVar('star_val')));
			$Return['csrf_hash'] = csrf_hash();
	...
	public function update_important_mail_record() {
		...
		if($this->request->getVar('field_id')) {
			...
			//$id = udecode($this->request->getGet('field_id'));
			$id = udecode(strip_tags(trim($request->getVar('field_id'))));
			$imp_val = strip_tags(trim($request->getVar('imp_val')));
			$Return['csrf_hash'] = csrf_hash();
