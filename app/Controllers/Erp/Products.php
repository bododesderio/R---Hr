	...
	public function add_product() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$category = strip_tags(trim($this->request->getPost('category')));
				$warehouse = strip_tags(trim($this->request->getPost('warehouse')));
				$barcode_type = strip_tags(trim($this->request->getPost('barcode_type')));
				$barcode = strip_tags(trim($this->request->getPost('barcode')));
				$sku = strip_tags(trim($this->request->getPost('sku')));
				$serial_number = strip_tags(trim($this->request->getPost('serial_number')));
				$qty = strip_tags(trim($this->request->getPost('qty')));
				$reorder_stock = strip_tags(trim($this->request->getPost('reorder_stock')));
				$expiration_date = strip_tags(trim($this->request->getPost('expiration_date')));
				$purchase_price = strip_tags(trim($this->request->getPost('purchase_price')));
				$selling_price = strip_tags(trim($this->request->getPost('selling_price')));
				$product_description = strip_tags(trim($this->request->getPost('product_description')));
	...
	public function update_rating() {
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			if(!$this->validate($rules)){
				...
				$product_rating = strip_tags(trim($this->request->getPost('product_rating')));
				$id = udecode(strip_tags(trim($this->request->getPost('token'))));
				$data = [
