...
class Leave extends BaseController {
	...
	// server-side list
	public function leave_list_server() {
		...
		$builder = $db->table('ci_leave_applications')
			->select('ci_leave_applications.*, ci_erp_users.first_name, ci_erp_users.last_name, ci_erp_users.email, ci_erp_users.profile_photo, ci_erp_constants.category_name as leave_type_name')
			->join('ci_erp_users', 'ci_erp_users.user_id = ci_leave_applications.employee_id', 'left')
			->join('ci_erp_constants', 'ci_erp_constants.constants_id = ci_leave_applications.leave_type_id', 'left');

		if($user_info['user_type'] == 'staff'){
			$builder->where('ci_leave_applications.employee_id', $usession['sup_user_id']);
		} else {
			$builder->where('ci_leave_applications.company_id', $usession['sup_user_id']);
		}
		...
		if (!empty($search)) {
			$builder->groupStart()
				...
				->orLike('ci_erp_constants.category_name', $search)
				->groupEnd();
		}

		$total    = $builder->countAllResults(false);
		$filtered = $builder->countAllResults(false);
		$data     = $builder->limit($length, $start)->orderBy('ci_leave_applications.leave_id', 'ASC')->get()->getResultArray();
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			$rules = [
				'employee_id' => [
					'rules'  => 'required',
					'errors' => [
						'required' => lang('Main.xin_staff_field_error')
					]
				],
				'leave_type' => [
					'rules'  => 'required',
			...
			if(!$this->validate($rules)){
			   ...
			   $validated = $this->validate([
				...
				$leave_type = strip_tags(trim($this->request->getPost('leave_type')));
				...
				$leave_half_day = strip_tags(trim($this->request->getPost('leave_half_day')));
				$remarks = strip_tags(trim($this->request->getPost('remarks')));
				$luser_id = strip_tags(trim($this->request->getPost('employee_id')));
				$UsersModel = new UsersModel();
				$ConstantsModel = new ConstantsModel();
				...
				if($user_info['user_type'] == 'staff'){
					...
					$staff_id = strip_tags(trim($this->request->getPost('employee_id')));
	...
	public function update_leave_status() {
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			if(!$this->validate($rules)){
				...
				$id = udecode(strip_tags(trim($this->request->getPost('token_status'))));	
				$data = [
					'remarks' => $remarks,
					'status'  => $status
				];
	...
	public function get_employee_assigned_leave_types()
	{
		...
		if($session->has('sup_username')){
			return view('erp/leave/get_employee_assigned_leave_types', $data);
		} else {
			return redirect()->to(site_url('erp/login'));
		}
	}
