	...
	public function add_ticket() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$ticket_priority = strip_tags(trim($this->request->getPost('ticket_priority')));
				...
				if($user_info['user_type'] == 'staff'){
					...
					if(strip_tags(trim($this->request->getPost('department_id'))) === ''){
						$Return['error'] = lang('Employees.xin_employee_error_department');
					} else if(strip_tags(trim($this->request->getPost('employee_id'))) === ''){
	...
	public function add_ticket_reply() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$token2 = udecode(strip_tags(trim($this->request->getPost('token2'))));	
				$data = [
					'company_id' => $company_id,
					'ticket_id' => $id,
					'sent_by'  => $usession['sup_user_id'],
