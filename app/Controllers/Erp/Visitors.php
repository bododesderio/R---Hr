...
class Visitors extends BaseController {
	...
	// server-side list
	public function visitors_list_server() {
		...
		$builder = $db->table('ci_visitors')
			->where('company_id', $company_id);

		if (!empty($search)) {
			$builder->groupStart()
				->like('visitor_name', $search)
				->orLike('visit_purpose', $search)
				->groupEnd();
		}

		$total    = $builder->countAllResults(false);
		$filtered = $builder->countAllResults(false);
		$data     = $builder->limit($length, $start)->orderBy('visitor_id', 'ASC')->get()->getResultArray();
	...
	public function add_visitor() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$visit_purpose = strip_tags(trim($this->request->getPost('visit_purpose')));
				$visitor_name = strip_tags(trim($this->request->getPost('visitor_name')));
				$visit_date = strip_tags(trim($this->request->getPost('visit_date')));
				$check_in = strip_tags(trim($this->request->getPost('check_in')));
				$phone = strip_tags(trim($this->request->getPost('phone')));
				$email = strip_tags(trim($this->request->getPost('email')));
				$address = strip_tags(trim($this->request->getPost('address')));
	...
	public function update_visitor() {
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			if(!$this->validate($rules)){
				...
				$visit_date = strip_tags(trim($this->request->getPost('visit_date')));
				$check_in = strip_tags(trim($this->request->getPost('check_in')));
				$check_out = strip_tags(trim($this->request->getPost('check_out')));
				$phone = strip_tags(trim($this->request->getPost('phone')));
				$email = strip_tags(trim($this->request->getPost('email')));
				$address = strip_tags(trim($this->request->getPost('address')));
				$description = strip_tags(trim($this->request->getPost('description')));
