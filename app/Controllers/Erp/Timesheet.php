...
class Timesheet extends BaseController {
	...
	/**
	 * Log attendance edit to audit trail (Phase 2.3)
	 */
	private function logAttendanceAudit(int $attendanceId, int $companyId, int $changedBy, string $field, $oldValue, $newValue): void
	{
		$db = \Config\Database::connect();
		$db->table('ci_attendance_audit')->insert([
	...
	/**
	 * Log all changed fields for an attendance record
	 */
	private function auditAttendanceChanges(int $attendanceId, int $companyId, int $changedBy, array $oldRecord, array $newData): void
	{
		$auditFields = ['attendance_date', 'clock_in', 'clock_out', 'total_work', 'attendance_status'];
		foreach ($auditFields as $field) {
			if (isset($newData[$field]) && isset($oldRecord[$field]) && $newData[$field] !== $oldRecord[$field]) {
				$this->logAttendanceAudit($attendanceId, $companyId, $changedBy, $field, $oldRecord[$field], $newData[$field]);
			}
		}
	}
	...
	// server-side list
	public function attendance_list_server() {
		...
		$builder = $db->table('ci_timesheet')
			->select('ci_timesheet.*, ci_erp_users.first_name, ci_erp_users.last_name, ci_erp_users.email, ci_erp_users.profile_photo')
			->join('ci_erp_users', 'ci_erp_users.user_id = ci_timesheet.employee_id', 'left')
			->where('ci_timesheet.company_id', $company_id);

		if (!empty($search)) {
			$builder->groupStart()
				->like('ci_erp_users.first_name', $search)
				->orLike('ci_erp_users.last_name', $search)
				->orLike('ci_timesheet.attendance_date', $search)
				->groupEnd();
		}

		$total    = $builder->countAllResults(false);
		$filtered = $builder->countAllResults(false);
		$data     = $builder->limit($length, $start)->orderBy('ci_timesheet.time_attendance_id', 'ASC')->get()->getResultArray();
	...
	public function add_attendance() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$attendance_date = strip_tags(trim($this->request->getPost('attendance_date_m')));
				$clock_in = strip_tags(trim($this->request->getPost('clock_in_m')));
				$clock_out = strip_tags(trim($this->request->getPost('clock_out_m')));
	...
	public function add_overtime() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$clock_in = strip_tags(trim($this->request->getPost('clock_in_m')));
				$clock_out = strip_tags(trim($this->request->getPost('clock_out_m')));
				$reason = strip_tags(trim($this->request->getPost('reason')));
	...
	public function update_attendance_record() {
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			if(!$this->validate($rules)){
				...
				$TimesheetModel = new TimesheetModel();
				// Audit trail: log old values before update
				$oldRecord = $TimesheetModel->find($id);
				if ($oldRecord) {
	...
	/**
	 * Check if GPS coordinates are within the company's geofence (Phase 2.4)
	 */
	private function withinGeofence(float $lat, float $lng): bool
	{
		...
		$maxRadius = (int)($settings['geofence_radius_m'] ?? 0);

		// If no office location configured, skip geofence check
	...
	public function set_clocking() {
		...
		if ($this->request->getPost('type') === 'set_clocking') {
			...
			$clock_state = strip_tags(trim($this->request->getPost('clock_state')));
			$latitude = strip_tags(trim($this->request->getPost('latitude')));
			$longitude = strip_tags(trim($this->request->getPost('longitude')));
			$time_id = strip_tags(trim($this->request->getPost('time_id')));
			...
			if($clock_state=='clock_in') {
				...
				// Geofence check (Phase 2.4)
				$geofence_flag = 0;
				$lat_dec = !empty($latitude) ? (float)$latitude : null;
				$lng_dec = !empty($longitude) ? (float)$longitude : null;
				if ($lat_dec !== null && $lng_dec !== null) {
					$geofence_flag = $this->withinGeofence($lat_dec, $lng_dec) ? 0 : 1;
				}
				...
				$data = [
					...
					'clock_in_latitude_dec' => $lat_dec,
					'clock_in_longitude_dec' => $lng_dec,
				];
				$TimesheetModel = new TimesheetModel();
				$result = $TimesheetModel->insert($data);	
				$Return['csrf_hash'] = csrf_hash();	
