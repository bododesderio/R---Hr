...
class Trackgoals extends BaseController {
	...
	public function goals_list() {
		...
		$data = array();
          ...
          foreach($get_data as $r) {
			...
			if($tracking_type){
				$itracking_type = $tracking_type['category_name'];
			} else {
				$itracking_type = '--';
			}
			/////
			$itype = $itracking_type;//.'<br><small class="text-muted"><i>'.$r['subject'].'<i></i></i></small>';
	...
	public function add_tracking() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$tracking_type = strip_tags(trim($this->request->getPost('tracking_type')));
				$subject = strip_tags(trim($this->request->getPost('subject')));
				$target_achiement = strip_tags(trim($this->request->getPost('target_achiement')));
	...
	public function update_rating() {
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			if(!$this->validate($rules)){
				...
				$goal_rating = strip_tags(trim($this->request->getPost('goal_rating')));
				$id = udecode(strip_tags(trim($this->request->getPost('token'))));
				$data = [
	...
	public function add_work() {
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			$goal_work = strip_tags(trim($this->request->getPost('goal_work')));	
