	...
	public function add_warning() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$warning_type = strip_tags(trim($this->request->getPost('warning_type')));
				$subject = strip_tags(trim($this->request->getPost('subject')));
				$warning_date = strip_tags(trim($this->request->getPost('warning_date')));
				$description = strip_tags(trim($this->request->getPost('description')));
				$warning_to = strip_tags(trim($this->request->getPost('warning_to')));
	...
	public function warning_list() {
		...
		$data = array();
          ...
          foreach($get_data as $r) {						
			...
			if($category_info){
				$itype = $category_info['category_name'];
			} else {
				$itype = '';
			}
			...
			if($warning_by){
				$iadded_by = $warning_by['first_name'].' '.$warning_by['last_name'];
			} else {
				$iadded_by = lang('Users.xin_user_data_removed');
			}
