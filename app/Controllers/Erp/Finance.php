	...
	public function deposit_list() {
		...
		$data = array();
          ...
          foreach($get_data as $r) {
			...
			if($payment_method){
				$ipayment_method = $payment_method['category_name'];
			} else {
				$ipayment_method = '';
			}
	...
	public function add_account() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$account_name = strip_tags(trim($this->request->getPost('account_name')));		
				$account_balance = strip_tags(trim($this->request->getPost('account_balance')));	
				$account_number = strip_tags(trim($this->request->getPost('account_number')));
				$branch_code = strip_tags(trim($this->request->getPost('branch_code')));
	...
	public function add_deposit() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$deposit_date = strip_tags(trim($this->request->getPost('deposit_date')));
				$category_id = strip_tags(trim($this->request->getPost('category_id')));
				$payer_id = strip_tags(trim($this->request->getPost('payer_id')));
				...
				$reference = strip_tags(trim($this->request->getPost('reference')));
