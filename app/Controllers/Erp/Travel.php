	...
	public function add_travel() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$visit_place = strip_tags(trim($this->request->getPost('visit_place')));
				$expected_budget = strip_tags(trim($this->request->getPost('expected_budget')));
				$actual_budget = strip_tags(trim($this->request->getPost('actual_budget')));	
		...
		$data = array();
          ...
          foreach($get_data as $r) {						
				...
				if($iuser_info){
					...
					$iemployee_name = lang('Users.xin_user_data_removed');
				}
			// get start date
