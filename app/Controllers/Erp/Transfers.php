...
class Transfers extends BaseController {
	...
	public function add_transfer() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$staff_id = strip_tags(trim($this->request->getPost('employee')));
				$department = strip_tags(trim($this->request->getPost('department')));
				$designation = strip_tags(trim($this->request->getPost('designation')));
				$transfer_date = strip_tags(trim($this->request->getPost('transfer_date')));
	...
	public function update_transfer() {
		...
		$request = \Config\Services::request();
		$usession = $session->get('sup_username');	
		if (strip_tags(trim($this->request->getPost('type'))) === 'edit_record') {
			$Return = array('result'=>'', 'error'=>'', 'csrf_hash'=>'');
			$Return['csrf_hash'] = csrf_hash();
			// set rules
			$rules = [
