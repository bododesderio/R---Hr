...
class Employees extends BaseController {
	...
	public function employees_list_server() {
		...
		$data     = $builder->limit($length, $start)->orderBy('user_id', 'ASC')->get()->getResultArray();
	...
	public function add_employee() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			$designation_id = strip_tags(trim($this->request->getPost('designation_id')));
			$basic_salary = strip_tags(trim($this->request->getPost('basic_salary')));
			$hourly_rate = strip_tags(trim($this->request->getPost('hourly_rate')));
			$salay_type = strip_tags(trim($this->request->getPost('salay_type')));
	...
	public function add_allowance() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$contract_tax_option = strip_tags(trim($this->request->getPost('contract_tax_option')));
				$is_fixed = strip_tags(trim($this->request->getPost('is_fixed')));
				$option_title = strip_tags(trim($this->request->getPost('option_title')));	
				$contract_amount = strip_tags(trim($this->request->getPost('contract_amount')));
	...
	public function add_document() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$document_name = strip_tags(trim($this->request->getPost('document_name')));
				$document_type = strip_tags(trim($this->request->getPost('document_type')));
				$id = udecode(strip_tags(trim($this->request->getPost('token'))));
	...
	public function update_company_info() {
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			$department_id = strip_tags(trim($this->request->getPost('department_id')));
			$designation_id = strip_tags(trim($this->request->getPost('designation_id')));
			$date_of_joining = strip_tags(trim($this->request->getPost('date_of_joining')));
			$date_of_leaving = strip_tags(trim($this->request->getPost('date_of_leaving')));
			$date_of_birth = strip_tags(trim($this->request->getPost('date_of_birth')));
			$marital_status = strip_tags(trim($this->request->getPost('marital_status')));
			$religion = strip_tags(trim($this->request->getPost('religion')));
			$blood_group = strip_tags(trim($this->request->getPost('blood_group')));
	...
	public function update_contract_info() {
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			//staff details
			$date_of_joining = strip_tags(trim($this->request->getPost('contract_date')));
			$department_id = strip_tags(trim($this->request->getPost('department_id')));
			$designation_id = strip_tags(trim($this->request->getPost('designation_id')));
			$basic_salary = strip_tags(trim($this->request->getPost('basic_salary')));
			$hourly_rate = strip_tags(trim($this->request->getPost('hourly_rate')));
			$salay_type = strip_tags(trim($this->request->getPost('salay_type')));
			$office_shift_id = strip_tags(trim($this->request->getPost('office_shift_id')));
			$role_description = strip_tags(trim($this->request->getPost('role_description')));
			$date_of_leaving = strip_tags(trim($this->request->getPost('contract_end')));
