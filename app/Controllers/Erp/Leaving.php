...
class Leaving extends BaseController {
	...
	{		
		...
		$data['breadcrumbs'] = lang('Dashboard.left_employees_exit');
	...
	public function add_exit() {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				...
				$exit_date = strip_tags(trim($this->request->getPost('exit_date')));
				$exit_type = strip_tags(trim($this->request->getPost('exit_type')));
				$exit_interview = strip_tags(trim($this->request->getPost('exit_interview')));
				$is_inactivate_account = strip_tags(trim($this->request->getPost('is_inactivate_account')));
